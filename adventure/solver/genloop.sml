...
struct
  ...
  structure GoodC = SolverFn(open GoodArg
                             val coalesce_missings = false) (* BROKEN *)
    ...
    let
      ...
      val (pf, (goal, pile)) = 
        let
          fun mktight () =
            let
              val (pf, (goal, pile)) = Generator.generate 0 seed LENGTH INVSIZE 
                (nil, (nil, [(Proof.Noun "Goal", nil)], Proof.Noun "Goal"));
            in
              (Good.provepuzzle NONE (goal, pile, INVSIZE - 1);
               (* loose! *)
               print "L";
               mktight ()
               ) handle Good.Solver _ =>  (pf, (goal, pile))
            end
        in
          mktight ()
        end
...
*)
      ...
      fun eval l = case List.mapPartial (fn {steps = n, ...} => n) l of
                        x :: _ => x
                      | nil => maxdepth
(*
        let
