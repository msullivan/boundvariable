...
type freelist = int Queue.t * int Stream.t (* queue is for free'd slots *)
...
(* only works by invariant! *)
let dynarray_set arr i v =
    try DynArray.set arr i v
    with
        DynArray.Invalid_arg (_, _, _) -> DynArray.add arr v

type state = { memory: (word array) DynArray.t;
               freelist: freelist;
               mutable ip: int;
               registers: word array; (* length = 8 *) }

let initial_state () =
    let memory = DynArray.make 100 in
    (* let () = Hashtbl.add memory 0l [| |] in (* XXX program? *) *)
    { memory = memory;
      freelist = (Queue.create (),
                  Stream.from (fun n -> Some (n + 1)));
      ip = 0;
...
let step state =
    let program = try DynArray.get state.memory 0
    ...
    match instr with
        ...
        | 1 ->
            let regs = regs word in
            let array =  try DynArray.get state.memory (word_to_int (lookup_reg regs.b))
                         with Not_found ->
        ...
        | 2 ->
            let regs = regs word in
            let array =  try DynArray.get state.memory (word_to_int (lookup_reg regs.a))
                         with Not_found ->
        ...
        | 8 ->
            ...
            dynarray_set state.memory id array;
            update_reg b (int_to_word id)
        | 9 ->
            let c = reg_c word in
            let id = word_to_int (lookup_reg c) in
            let () = if id = 0
                     then raise (Fail "program attempted to free array 0")
            in
            dynarray_set state.memory id [| |];
        ...
        | 12 ->
            let b = reg_b word in
            let c = reg_c word in
            let id = word_to_int (lookup_reg b) in
            let array =  try DynArray.get state.memory id
                         with DynArray.Invalid_arg (_, _, _) ->
                            raise (Fail "invalid array in Iload_program")
            in
            (* optimization: if it's 0, it's already loaded *)
            let array = if id = 0 then array else Array.copy array
            in
            DynArray.set state.memory 0 array;
...
let load_program_from_file state file =
    ...
    DynArray.add state.memory array (* XXX adds at zero *)
