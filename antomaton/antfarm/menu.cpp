...
void textinput::draw(int xpos, int ypos, int f) {
  string i = input;
...
void menu::draw() {
  ...
  /* XXX draw a "less" icon if skip != 0 */
...
/* set skip according to the width, height, and
   selected control. */
void menu::fixup() {
  /* start with skip = 0.
     ...
     this can only fail if the selected control itself is bigger than
     the screen. Just to avoid looping forever here, stop if skip =
     selected; we can't give it any more room than that!
  */
  ...
  for(;;) {
    ...
    for(int z = skip; z < nitems;) {
      int ww, hh;
      items[z]->size(ww, hh); ww += items[z]->indent;
	...
	/* the control doesn't fit, and
	   we haven't found the selected
	   control yet, so fail. */
	skip ++;
	goto tryagain;
      }
